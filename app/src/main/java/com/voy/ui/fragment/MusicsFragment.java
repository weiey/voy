...
import com.voy.player.MusicPlayService;
import com.voy.player.MusicPlayState;
import com.voy.presenter.MusicsPresenter;
import com.voy.presenter.impl.MusicsPresenterImpl;
import com.voy.ui.fragment.base.BaseFragment;
import com.voy.utils.CommonUtils;
import com.voy.view.MusicsView;
import com.voy.widgets.PlayerDiscView;
...
//        toggleShowLoading(true, null);
        ...
        if (NetUtils.isNetworkConnected(mContext)) {
            mMusicsPresenter.loadListData(TAG_LOG, mMusicsCollectId, AppConstants.Events.EVENT_REFRESH_DATA);
        } else {
...
//            });
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        bundleFilter.addAction(AppConstants.ACTION_MUSIC_BUNDLE_BROADCAST);
        ...
        posFilter.addAction(AppConstants.ACTION_MUSIC_CURRENT_PROGRESS_BROADCAST);
        ...
        secondProgressFilter.addAction(AppConstants.ACTION_MUSIC_SECOND_PROGRESS_BROADCAST);
    ...
    protected void initView() {
        ...
        mBackgroundImage = $(R.id.musics_player_background);
        mPlayerDiscView =$(R.id.musics_player_disc_view);
        mPlayerCtrlBtn = $(R.id.musics_player_play_ctrl_btn);
        mPlayerNextBtn =$(R.id.musics_player_play_next_btn);
        mPlayerPrevBtn =  $(R.id.musics_player_play_prev_btn);
        mPlayerSeekBar = $(R.id.musics_player_seekbar);
        mTitle = $(R.id.musics_player_name);
        mSonger = $(R.id.musics_player_songer_name);
        mCurrentTime = $(R.id.musics_player_current_time);
        mTotalTime = $(R.id.musics_player_total_time);
         mLoadingTargetView = $(R.id.musics_player_loading_view);


        mBackgroundImage.setImageBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.player_bg));
...
//    }
    ...
    public void playNextMusic() {
        ...
        mPlayerCtrlBtn.setImageResource(R.drawable.btn_play_selector);
        mMusicsPresenter.loadListData(TAG_LOG, mMusicsCollectId, AppConstants.Events.EVENT_LOAD_MORE_DATA);
    }
    ...
    public void seekToPosition(int position) {
        mContext.sendBroadcast(new Intent(MusicPlayState.ACTION_SEEK_TO).putExtra(AppConstants.KEY_PLAYER_SEEK_TO_PROGRESS, position));
    }
...
//        toggleShowLoading(false, null);
        ...
        if (!TextUtils.isEmpty(imageUrl)) {
            ...
            ImageLoader.getInstance().loadImage(imageUrl, new ImageLoadingListener() {
                ...
                public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
                    mBackgroundImage.setImageBitmap(loadedImage);
                }
            ...
            mBackgroundImage.setImageBitmap(BitmapFactory.decodeResource(getResources(),
    ...
    private class PlayBundleBroadCast extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (null != action && !TextUtils.isEmpty(action)) {
                if (action.equals(AppConstants.ACTION_MUSIC_BUNDLE_BROADCAST)) {
                    Bundle extras = intent.getExtras();
                    if (null != extras) {
                        MusicsListEntity entity = extras.getParcelable(AppConstants.KEY_MUSIC_PARCELABLE_DATA);
                        int totalDuration = extras.getInt(AppConstants.KEY_MUSIC_TOTAL_DURATION);
    ...
    private class PlayPositionBroadCast extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (null != action && !TextUtils.isEmpty(action)) {
                if (action.equals(AppConstants.ACTION_MUSIC_CURRENT_PROGRESS_BROADCAST)) {
                    Bundle extras = intent.getExtras();
                    if (null != extras) {
                        int progress = extras.getInt(AppConstants.KEY_MUSIC_CURRENT_DUTATION);
    ...
    private class PlaySecondProgressBroadCast extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (null != action && !TextUtils.isEmpty(action)) {
                if (action.equals(AppConstants.ACTION_MUSIC_SECOND_PROGRESS_BROADCAST)) {
                    Bundle extras = intent.getExtras();
                    if (null != extras) {
                        int progress = extras.getInt(AppConstants.KEY_MUSIC_SECOND_PROGRESS);
