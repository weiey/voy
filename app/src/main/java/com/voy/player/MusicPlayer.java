...
public class MusicPlayer implements OnCompletionListener, OnErrorListener, OnBufferingUpdateListener, OnPreparedListener {
    ...
    public void onBufferingUpdate(MediaPlayer mp, int percent) {
        if (percent < 100) {
            ...
            intent.setAction(AppConstants.ACTION_MUSIC_SECOND_PROGRESS_BROADCAST);
            intent.putExtra(AppConstants.KEY_MUSIC_SECOND_PROGRESS, percent);
    ...
    private void sendPlayBundle() {
        Intent intent = new Intent(AppConstants.ACTION_MUSIC_BUNDLE_BROADCAST);
        Bundle extras = new Bundle();
        extras.putInt(AppConstants.KEY_MUSIC_TOTAL_DURATION, getDuration());
        extras.putParcelable(AppConstants.KEY_MUSIC_PARCELABLE_DATA, mMusicList.get(mCurPlayIndex));
    ...
    private void sendPlayCurrentPosition() {
        new Thread() {
            public void run() {
                Intent intent = new Intent(AppConstants.ACTION_MUSIC_CURRENT_PROGRESS_BROADCAST);
                while (mMediaPlayer.isPlaying()) {
                    intent.putExtra(AppConstants.KEY_MUSIC_CURRENT_DUTATION, getCurPosition());
                    mContext.sendBroadcast(intent);
                    try {
                        sleep(SLEEP_TIME);
                    } catch (Exception e) {
